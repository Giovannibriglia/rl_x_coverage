...
class MarlMAPPO(MarlBase):
    ...
    def train_and_evaluate(
        ...
        for it, data in enumerate(self.collector):
            ...
            rewards_np = ep_rew.squeeze(-1).cpu().numpy()
            eta_np = eta_td.squeeze(-1).cpu().numpy() if eta_td is not None else None
            beta_np = beta_td.squeeze(-1).cpu().numpy() if beta_td is not None else None
            collisions_np = (
                coll_td.squeeze(-1).cpu().numpy() if coll_td is not None else None
            )
            ...
            if it in checkpoint_iters:
                ...
                pbar.set_postfix(checkpoint=f"{n_chkpt}/{n_checkpoints_metrics}")
                ...
                for env_idx, (env_test_name, env_test_obj) in enumerate(
                    ...
                    pbar.set_postfix(env_test=f"{env_idx}/{len(envs_test)}")
